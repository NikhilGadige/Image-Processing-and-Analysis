...
for i=1:rows
    for j=1:cols
        ...
        if V(i,j)>=A && V(i,j)<=B
            slice(i,j) = 1;        % highlight range
        else
            slice(i,j) = V(i,j);   % keep original background
        end
...
% Convert histogram into an image
...
% Resize to match image display size
...
% Color Space: RGB
% Reason: simulate blur reduction step
%% ==========================================================
